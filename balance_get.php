...
for($i = 0; $i<count($files); $i++){
	exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c true > /dev/null 2>&1 &');
	...
	if($btc >= 0.00003){
		exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c @BitcoinClick_bot -a '.$btc.' > /dev/null 2>&1 &');
		exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c true > /dev/null 2>&1 &');
		send_message(urlencode('С аккаунта: '.$files[$i].' было выведено: '.$btc.' BTC'));
	}
	if($ltc >= 0.0004){
		exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c @Litecoin_click_bot -a '.$ltc.' > /dev/null 2>&1 &');
		exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c true > /dev/null 2>&1 &');
		send_message(urlencode('С аккаунта: '.$files[$i].' было выведено: '.$ltc.' LTC'));
	}

	if($bch >= 0.0001){
		exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c @BCH_clickbot -a '.$bch.' > /dev/null 2>&1 &');
		exec('nohup php /var/www/clicker/prod/'.$files[$i].'/balance-cli.php -c true > /dev/null 2>&1 &');
